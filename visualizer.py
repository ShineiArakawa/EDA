...
def main():
    ...
    dataset_pickle_path = 'data/dataset_for_visualize.pkl'
    ...
    logger.info(
        f'one_data["point_clouds"].shape                : {one_data["point_clouds"].shape}')
    logger.info(
        f'one_data["og_color"].shape                    : {one_data["og_color"].shape}')
    logger.info(
        f'one_data["utterances"]                        : {one_data["utterances"]}')
    logger.info(
        f'one_data["all_detected_boxes"].shape          : {one_data["all_detected_boxes"].shape}')
    logger.info(
        f'one_data["all_detected_bbox_label_mask"].shape: {one_data["all_detected_bbox_label_mask"].shape}')
    logger.info(
        f'one_data["all_detected_class_ids"].shape      : {one_data["all_detected_class_ids"].shape}')
    logger.info(
        f'one_data["target_id"]                         : {one_data["target_id"]}')
    logger.info(
        f'one_data["target_name"]                       : {one_data["target_name"]}')
    logger.info(
        f'len(anchor_bbox_corners)                      : {len(anchor_bbox_corners)}')
    logger.info(
        f'len(distractor_bbox_corners)                  : {len(distractor_bbox_corners)}')
    logger.info(
        f'bbox_corners.shape                            : {bbox_corners.shape}')
